...
def get_opt_parse():
    ...
    action_group.add_option(
        ...
        const=Action.PRINT,
        dest="action",
        help="Print out contents of target Zookeeper location.",
    )
    action_group.add_option(
        "-c",
        "--copy",
    ...
    action_group.add_option(
        ...
        const=Action.EXPORT,
        dest="action",
        help="Write contents of target Zookeeper location to local JSON file.",
    )
    action_group.add_option(
        "-i",
        "--import",
    ...
    # Resolution
    resolution_group = OptionGroup(
        parser,
        "Resolution options: specify how to deal with conflicts. (Default: --no-clobber)",
    )
    resolution_group.add_option(
        "--no-clobber",
        action="store_const",
        const=Resolve.NO_CLOBBER,
        dest="resolve",
        help="Do not overwrite any existing ZNodes.",
    )
    resolution_group.add_option(
        "--interactive",
        action="store_const",
        const=Resolve.INTERACTIVE,
        dest="resolve",
        help="Prompt user for each conflict encountered.",
    )
    resolution_group.add_option(
        "--overwrite",
        action="store_const",
        const=Resolve.OVERWRITE,
        dest="resolve",
        help="Overwrite existing ZNodes without prompting user.",
    )
...
class ZkTreeUtil(object):
    def __init__(self):
        ...
        if self.option.file_loc and (
            self.option.action == Action.PRINT or self.option.action == Action.COPY
        ):
    ...
    def traverse_zk_tree(
        ...
        print(src_zk_client.get_children(path))
        ...
        if dest_zk_client.exists(dest_znode_path):
            if resolve == Resolve.INTERACTIVE:
                ...
                while string.lower(response) != "y" and string.lower(response) != "n":
                    response = input(
                        "ZNode at %s already exists at destination. Overwrite? (y/n)"
                        % dest_znode_path
                    )
                ...
                self.logger.debug(
                    "ZNode at %s already exists. Overwriting data due to --overwrite"
                    % dest_znode_path
                )
                dest_zk_client.set(dest_znode_path, znode.data)
                ...
                self.logger.debug(
                    "ZNode at %s already exists. Skipping due to --no-clobber"
                    % dest_znode_path
                )
        else:
    ...
    def run_copy(self, source_zk, source_zk_path, dest_zk, dest_zk_path, resolve):
        ...
        self.traverse_zk_tree(
            source_zk_client,
            source_zk_path,
            self.process_znode_write_to_zk,
            dest_zk_client=dest_zk_client,
            dest_zk_path=dest_zk_path,
            resolve=resolve,
        )
    ...
    def run_print(self, source_zk, source_zk_path):
        source_zk_client = create_zk_client(source_zk)
        self.traverse_zk_tree(
            source_zk_client, source_zk_path, self.process_znode_print
        )
